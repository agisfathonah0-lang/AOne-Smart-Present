...
const LoginScreen = () => {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    setStatusLog('Memulai proses login...');

    try {
      const result = await login(email.trim(), password, (msg) => {
        setStatusLog(msg);
      });
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          if (result.user.role === 'admin') {
            ...
            Alert.alert('Akses Ditolak', 'Role tidak dikenali.');
          }
        }, 500);
      } else {
...
const styles = StyleSheet.create({
  ...
  title: {
    ...
    color: '#1e293b', letterSpacing: 1, textTransform: 'uppercase'
  },
  subtitle: {
    fontSize: 11, textAlign: 'center',
    color: '#64748b', marginBottom: 25, fontWeight: '600',
  },
  inputGroup: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: '#f8fafc', paddingHorizontal: 15,
  ...
  logText: {
    ...
    color: '#475569', fontWeight: '600', flex: 1
  },
  button: {
    backgroundColor: Theme.primary || '#22d3ee',
    padding: 18, borderRadius: 14, alignItems: 'center', marginTop: 5,
    shadowColor: Theme.primary || '#22d3ee',
  ...
  footerText: { marginTop: 25, textAlign: 'center', color: '#94a3b8', fontSize: 10, fontWeight: '600' }
});
